...
exports.entryAsync = async(groupId, memberId, authId) => {
  ...
  // FIXME: set memberId related with groupId
  const userInfo = await getMemberAsync(groupId, await Auth.getMemberIdAsync(groupId, authId));
...
exports.findGroupsByAuthIdAsync = async(authId) => {
  const userId = await Auth.getMemberIdAsync('sla', authId);
