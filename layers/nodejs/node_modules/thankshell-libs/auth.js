...
exports.isAccessableAsync = async(groupId, permissions, claims) => {
  const userInfo = await exports.getMemberAsync(groupId, await exports.getMemberIdAsync(groupId, exports.getAuthId(claims)));
  ...
  if (permissions.includes('members') && group.members.values.includes(userId)){
    return true;
  }

  if (permissions.includes('admins') && group.admins.values.includes(userId)){
    return true;
  }
